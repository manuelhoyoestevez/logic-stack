...
import mhe.proxy.DispatcherInterface;
...
public class Dispatcher implements DispatcherInterface {
	...
	protected Future<Void> dispatchServiceLessRequest(RoutingContext originalRequest, String jwt, LoggerInterface parentLogger) {
		...
		fut.fail(new MheException(400, "SERVICE_ID_NOT_RESOLVED", "Service ID not resolved"));
		return fut;
	}
						...
						this.serviceCatalog.getNextServiceParams(serviceId, requestFactory).setHandler(requestedServiceResult -> {
							try {
								...
								if(requestedServiceResult.failed()) {
									...
									fut.fail(new MheException(500, "SERVICE_NOT_INSTACIATED", "Service with ID '" + serviceId + "' not instanciated"));
								}
								else if(!serviceParams.isAllowed()) {
									fut.fail(new MheException(403, "SERVICE_ID_FORBIDDEN", "Service ID forbidden"));
								}
								else {
