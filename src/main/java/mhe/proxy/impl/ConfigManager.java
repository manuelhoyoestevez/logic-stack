...
import mhe.consul.ConsulConfigManagerHelper;
import mhe.consul.ConsulServiceCatalog;
import mhe.logger.LoggerInterface;
import mhe.proxy.AuthenticatorInterface;
import mhe.proxy.AuthorizatorInterface;
import mhe.proxy.ConfigManagerInterface;
import mhe.proxy.ConfigParamsInterface;
import mhe.proxy.auth.Authenticator;
import mhe.proxy.auth.Authorizator;
import mhe.proxy.auth.JWTDecoder;
import mhe.proxy.auth.JWTDecoderInterface;
import mhe.proxy.impl.ConfigParams;
import mhe.common.FutureChain;
			...
			try {
				...
				for(String requiredParameterKey : requiredParameters){
					...
					if(requiredParameterValue == null) {
						throw new MheException(500, "MISSING_PARAMETER", "Missing parameter '" + requiredParameterKey + "'");
					}
					else {
