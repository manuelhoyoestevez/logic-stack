...
import mhe.logger.Logger;
import mhe.logger.LoggerInterface;
import mhe.proxy.impl.ConfigManager;
...
public class ProxyVerticle extends AbstractVerticle {
										...
										if(acceptedRequestResult.failed()) {
											...
											switch(action) {
												...
												default:
													...
													this.returnFail(new MheException(500, "UNKNOWN_FIREWALL_ACTION", "Action unknown: " + action), originalRequest, requestFactory);
											}
										}
									}
									catch(Throwable ex) {
	...
	private void returnFail(Throwable cause, RoutingContext routingContext, LoggerInterface requestFactory){
		...
		if(cause instanceof MheException) {
			MheException exception = (MheException) cause;
